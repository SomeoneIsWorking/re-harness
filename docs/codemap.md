...
| C++ policy check | Effective clang-tidy/clang-format config audit and compile-database-backed AST scan of global functions, extern declarations, and block-scope static/const variables; projects own exact exclusions and ABI entry points | `tools/cpp_policy.py` | `main()`, `inspect_configs()`, `inspect_ast()` | `README.md` |
