...
| C++ policy check | Effective clang-tidy/clang-format config audit and compile-database-backed AST scan of global functions, extern declarations, and function-local statics, one process per translation unit in parallel, reading Clang's JSON by token rather than by character; projects own exact exclusions and ABI entry points | `tools/cpp_policy.py` | `main()`, `inspect_configs()`, `check_database()`, `check_unit()`, `apply_accepted()` | `README.md` |
