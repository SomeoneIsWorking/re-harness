#!/usr/bin/env python3
"""catalog.py — a tiny, dependency-free atomic-work and issue catalog.

A greppable registry of atomic tasks, bugs, investigations, blockers, findings,
root causes, and DEAD ENDS for a codebase. One Markdown file per entry under a
catalog dir (default `docs/issues/`), each with a small frontmatter block so it
stays human-readable and `grep`-able even without this tool. Entries can link to
project-state items without becoming the state inventory.
...
Zero dependencies (Python 3.8+ stdlib only). Run from a project root:
    ...
    catalog.py add "shader colors wrong" --symptom "creature meshes render grey" \
        --state-item S014 --tags render,material --status open
...
def _default_dir():
    ...
    while True:
        candidate = os.path.join(here, "docs", "issues")
    ...
    try:
        ...
        if top.returncode == 0 and top.stdout.strip():
            return os.path.join(top.stdout.strip(), "docs", "issues")
...
def _state_item_id(text):
    value = text.upper()
    if not re.fullmatch(r"S\d+", value):
        raise argparse.ArgumentTypeError("must be an ID such as S014")
    return value
...
def _parse(path):
    ...
    meta["state_items_list"] = [s.upper() for s in re.split(r"[,\s]+", meta.get("state_items", "")) if s]
    meta["path"] = path
    meta["body"] = body.strip()
    return meta


def _load_all(cdir, *, refuse_if_missing=True):
    ...
    if not os.path.isdir(cdir):
        if not refuse_if_missing:
            return []
        sys.exit(f"catalog: {os.path.abspath(cdir)} does not exist — this searched NOTHING. "
                 f"(Set $CATALOG_DIR or --dir, or `catalog.py add --init` to create it.)")
    out = []
...
def _write(cdir, meta, body):
    order = ["id", "title", "status", "symptom", "state_items", "tags", "created", "updated"]
...
def cmd_add(args):
    ...
    if not os.path.isdir(cdir):
        if not args.init:
            sys.stderr.write(
                f"catalog: {cdir} does not exist, so this would START A NEW CATALOG rather "
                f"than add to one.\n"
                f"catalog: cwd is {os.getcwd()}; pass --dir for an existing catalog or "
                f"--init to create this one.\n")
            raise SystemExit(2)
    ...
    meta = {
        ...
        "state_items": ",".join(args.state_items or []),
...
def _score(e, terms):
    hay = {
        ...
        "state_items": " ".join(e["state_items_list"]).lower(),
        "tags": " ".join(e["tags_list"]).lower(), "body": e["body"].lower(),
    }
    weights = {"symptom": 5, "title": 4, "state_items": 3, "tags": 3, "body": 1}
    score = 0
    for t in terms:
...
def _print_row(e, extra=""):
    ...
    state_items = ",".join(e["state_items_list"])
    print(f"  #{e['id_num']:<4} [{e.get('status','?'):<13}] {e.get('title','')}"
          + (f"  <{state_items}>" if state_items else "")
...
def cmd_list(args):
    ...
    if args.state_item:
        entries = [e for e in entries if args.state_item in e["state_items_list"]]
...
def main(argv=None):
    p = argparse.ArgumentParser(description="tiny atomic-work and issue catalog")
    ...
    a.add_argument("--init", action="store_true",
                   help="allow creating the catalog directory")
    a.add_argument("--symptom", help="the observable symptom (search-weighted highest)")
    a.add_argument("--state-item", dest="state_items", action="append", default=[],
                   type=_state_item_id,
                   help="related project-state ID such as S014; repeatable")
    a.add_argument("--tags", "--tag", dest="tags", action="extend", default=[],
                   type=lambda s: [t for t in re.split(r"[,\s]+", s) if t],
                   help="comma/space separated; repeatable, and repeats accumulate")
    ...
    l.add_argument("--tag")
    l.add_argument("--state-item", type=_state_item_id,
                   help="only issues linked to this project-state ID")
