...
def check_paths(objects, cwd):
    ...
    for sha, path in objects:
        ...
        try:
            ...
            continue
        # BINARIES ARE SCANNED TOO, and skipping them was this check's worst
        # failure mode: a committed .pyc records the ABSOLUTE PATH of the source
        # it was compiled from, so it is the single most likely file in a repo to
        # carry /home/<user> -- and being neither in TEXT_EXTS nor free of NUL
        # bytes, it was skipped twice over. A real one sat in a scanned repo
        # through 340 commits while this check reported 55 findings elsewhere and
        # never mentioned it.
        #
        # Their strings are extracted rather than the bytes decoded as lines: a
        # binary has no lines, and splitting NUL-laden data on newlines yields
        # megabyte "lines" whose snippets are unreadable.
        binary = not is_text_path(path) or b"\x00" in data[:8192]
        if binary:
            text = "\n".join(
                m.decode("ascii") for m in
                re.findall(rb"[\x20-\x7e]{6,}", data))
        else:
