...
WHY THIS EXISTS. A project accumulates durable knowledge at distinct levels that ALREADY have homes:
  · epic intent -> success condition docs/project-goals.md     (project-goals skill)
  · capability/outcome -> state      docs/project-state.md     (project-state skill)
  · atomic point -> cause/dead-end    docs/issues/              (issue-catalog skill, catalog.py)
  · subsystem -> owner/location      docs/codemap.md            (codemap skill, codemap.py)
...
USAGE
  info.py brief <words...>      # START HERE. One query across goals, project state, issues, claims,
                                # instruments, codemap, and re-frontier.
...
REAL_TOOL_DIR = os.path.dirname(os.path.realpath(__file__))
if REAL_TOOL_DIR not in sys.path:
...
def funcname_pattern(sym):
    """git's `-L:<funcname>:<file>` splits its argument ON COLONS, so a C++ qualified name is
    unparseable there: `-L:Window::run:src/app/window.cpp` makes git complain about a path named
    `:run:src/app/window.cpp` and exit non-zero. sym_log then falls back to FILE scope — silently,
    and while this tool's own advice is "narrow it with `--depends path.cpp#symbol`". For a codebase
    of C++ methods that advice could not be followed, and the claim sat in COARSE forever looking
    like the author's omission.

    funcname is a REGEX, so `.` stands in for each colon: `Window..run` contains no colon for git to
    split on and still anchors on the CLASS. Dropping the prefix and matching bare `run` was the
    other option and is worse — in a file with two classes it would silently measure the wrong
    method, which is the false-FRESH this checker exists to prevent."""
...
def sym_log(repo, rel, sym):
    ...
    def run():
        pat = funcname_pattern(sym)
        rc, out = git(repo, "log", f"-L:{pat}:{rel}", "-s", "--format=%ct%x09%h%x09%ci%x09%s")
...
def staleness_strength(row):
    """"symbol" if a SYMBOL this claim names moved, else "file". THE definition — module scope on
    purpose. The selftest first carried its own copy of this lambda and a mutation to the reporting
    side passed the selftest clean: it was asserting against a duplicate, not against the shipping
    code. One definition, both callers."""
    return "symbol" if any(d.scope == "symbol" for d, *_ in row["changed"]) else "file"
...
def cmd_claim_check(a):
    ...
    checked = fresh + stale
    # STRENGTH OF EVIDENCE, SPLIT — because for months this printed 22 STALE out of 37 and every
    # single one of them came from the weakest signal it has. Measured on 2026-08-12: 36 staleness
    # hits, 36 of them file-scope, ZERO symbol-scope. A file-scope dep says "some commit touched a
    # file this claim names"; src/main.cpp alone had moved 39 times. That cannot distinguish an
    # unrelated edit from a real invalidation, so an alarm built on it fires forever and gets
    # ignored — which is what happened. Symbol scope CAN distinguish them (the --selftest's negative
    # case is exactly "file edited, function not" -> fresh), so the two are now reported apart:
    # symbol-scope staleness is the actionable list and sets the exit status; file-scope is a
    # counted, named advisory that says plainly it is not evidence of rot.
    stale_sym = [r for r in stale if staleness_strength(r) == "symbol"]
    stale_file = [r for r in stale if staleness_strength(r) == "file"]
    ...
    if stale_file:
        print(f"COARSE  {len(stale_file)} claim(s) name a whole FILE that has moved, and nothing "
              f"finer:\n"
              f"        {', '.join(r['id'] for r in stale_file)}\n"
              f"        THIS IS NOT EVIDENCE THEY ROTTED. A file-scope dependency cannot tell an\n"
              f"        unrelated edit from a real invalidation — one of these names src/main.cpp,\n"
              f"        which moves most sessions. Narrow one with `--depends path.cpp#symbol` (or\n"
              f"        edit its `depends:` line) and it becomes a real signal; leave it and it\n"
              f"        will be listed here forever. Not counted as stale and does not fail this\n"
              f"        check, because an alarm that always fires is not an alarm.\n")
    ...
    print(f"     STALE (a cited SYMBOL moved) {len(stale_sym)}   <- the actionable list")
    print(f"     coarse (only a whole file) . {len(stale_file)}  advisory; see COARSE above")
...
def selftest():
    ...
    # TWO CLASSES WITH A METHOD OF THE SAME NAME, for the C++ qualified-name cases (C005/C006).
    # The name collision is the point: it is what distinguishes anchoring on `Window::run` from
    # giving up on the class and matching bare `run`.
    w("src/win.cpp", "void Window::run(){ int a=1; }\nvoid Other::run(){ int b=2; }\n")
    ...
    w("docs/info/claims/002-fresh-case.md",
      ...
      "field for field.\n")
    # C004: declares a bare FILE that DID move, and its prose names no symbol. Before the
    # file/symbol split this was reported identically to C001 and made the check unusable — 22 of
    # 37 real claims were exactly this shape. It must now be COARSE: listed, not counted as stale,
    # and NOT enough to fail the check.
    w("docs/info/claims/004-file-scope-only.md",
      "---\nid: C004\nkind: claim\nstatus: holds\ncreated: 2020-01-02\n"
      "depends: src/boot.cpp\n---\n\n## Claim\n\nThe boot path is fine overall.\n\n"
      "## Evidence\n\nI read the whole file and it hung together.\n")
    # C005/C006: C++ QUALIFIED NAMES, both directions. git's `-L:<funcname>:<file>` splits on
    # colons, so `Window::run` used to make git exit non-zero and the checker fall back to FILE
    # scope — silently, while printing "narrow it with path#symbol" as the remedy. C005 must be
    # stale AT SYMBOL SCOPE (the qualified name resolves at all) and C006 must be FRESH (it anchors
    # on the right CLASS: bare `run` would match the edited Window::run in the same file and report
    # a change to a method nobody touched).
    w("docs/info/claims/005-qualified-stale.md",
      "---\nid: C005\nkind: claim\nstatus: holds\ncreated: 2020-01-02\n"
      "depends: src/win.cpp#Window::run\n---\n\n## Claim\n\nThe window loop ticks once per frame.\n"
      "\n## Evidence\n\nRead the loop.\n")
    w("docs/info/claims/006-qualified-fresh.md",
      "---\nid: C006\nkind: claim\nstatus: holds\ncreated: 2020-01-02\n"
      "depends: src/win.cpp#Other::run\n---\n\n## Claim\n\nThe other runner is unchanged.\n"
      "\n## Evidence\n\nRead it too.\n")
    ...
    # Edit Window::run and leave Other::run alone, in the SAME file.
    w("src/win.cpp", "void Window::run(){ int a=1; tick(); }\nvoid Other::run(){ int b=2; }\n")
    ...
    try:
        ...
        # THE FILE-vs-SYMBOL SPLIT, both directions. C004 is stale only because a whole file it
        # names moved; it must NOT reach the actionable list, or the list goes back to being 22
        # entries nobody reads. C001 must still reach it, or quieting the noise has silenced the
        # signal too — which is the strictly worse failure and the reason both are asserted here.
        strongest = staleness_strength      # the SHIPPING one, not a copy
        print(f"  file-scope-only C004 (bare file moved)         -> {kind.get('C004')}, "
              f"strength {strongest(detail['C004']) if kind.get('C004') == 'stale' else 'n/a'}")
        if kind.get("C004") != "stale":
            fails.append(f"C004 MUST still be detected as stale, got {kind.get('C004')} — the "
                         f"coarse signal was removed rather than demoted")
        elif strongest(detail["C004"]) != "file":
            fails.append("C004's staleness was attributed to a SYMBOL it does not name, so the "
                         "split cannot demote anything")
        if kind.get("C001") == "stale" and strongest(detail["C001"]) != "symbol":
            fails.append("C001's staleness is not attributed to a symbol, so the actionable list "
                         "would be EMPTY and a real invalidation would be reported as advisory")

        # C++ QUALIFIED NAMES, both classes. Either failure is silent in production: C005 wrong
        # means every method-scoped claim quietly degrades to file scope, C006 wrong means the
        # checker reports rot in a method nobody edited.
        print(f"  qualified C005 (Window::run edited)             -> {kind.get('C005')}, "
              f"strength {strongest(detail['C005']) if kind.get('C005') == 'stale' else 'n/a'}")
        print(f"  qualified C006 (Other::run untouched, same file)-> {kind.get('C006')}")
        if kind.get("C005") != "stale":
            fails.append(f"C005 MUST be stale, got {kind.get('C005')} — a C++ qualified name does "
                         f"not resolve at all and the edit to the method it names was not seen")
        elif strongest(detail["C005"]) != "symbol":
            fails.append(f"C005 is stale only at {strongest(detail['C005'])} scope — the qualified "
                         f"name fell back to the whole FILE, which is the silent degradation this "
                         f"fixture exists to catch, and the tool's own '#symbol' advice is a lie")
        if kind.get("C006") != "fresh":
            fails.append(f"C006 MUST be fresh, got {kind.get('C006')} — the pattern is not anchored "
                         f"on the CLASS, so it matched the edited Window::run in the same file and "
                         f"is reporting rot in a method nobody touched")
    ...
    if fails:
        ...
        for f in fails:
            print(f"  - {f}")
        return 1
    print("\n  selftest PASSED: symbol-scope change flagged as ACTIONABLE, whole-file-only change "
          "demoted to advisory, unrelated churn not flagged, blind counted separately, "
...
def _grep_dir(d, words, label, limit=6):
    ...
    hits = []
    for e in entries(d):
        # THE FRONT MATTER IS PART OF THE ENTRY AND MUST BE SEARCHED. This line used to read
        # `" ".join(e.values() if False else [])` — a disabled expression that always joined an
        # empty list — so only the body was ever indexed and `tags:` was invisible. The effect was
        # the worst one this tool has: `brief hooks` returned an EMPTY claims section while C023,
        # whose front matter is literally `tags: hooks,timing`, sat on disk governing the whole
        # subsystem. An empty section reads as "nothing has been proven here", which was the exact
        # opposite of the truth, at the one moment the tool is consulted.
        meta = " ".join(str(v) for k, v in e.items() if not k.startswith("_"))
        blob = (e["_body"] + " " + meta).lower()
        score = 0
        for w in words:
            wl = w.lower()
            n = blob.count(wl)
            if n == 0 and len(wl) > 3:
                # Fold the trivial plural both ways. "overrides"/"override" and "hooks"/"hook" are
                # the same query to a human, and a registry that only answers one spelling is a
                # registry that answers "nothing known" to half the people who ask.
                alt = wl[:-1] if wl.endswith("s") else wl + "s"
                n = blob.count(alt)
            score += n
        if score:
            hits.append((score, e))
...
def cmd_brief(a):
    ...
    emit_external_sources(ROOT, SKILLS, words)
    ...
    if tc or ti:
        ...
        print("\n  THE CLAIMS AND INSTRUMENTS LEDGERS DO NOT EXIST IN THIS "
              ...
              "`info.py instrument add`, and note that any PROJECT GOALS/PROJECT "
              "STATE/ISSUES/CODEMAP/RE-FRONTIER sections above are the only registries that were "
              "actually consulted.")
    else:
...
def cmd_check(a):
    ...
    if not a.no_stale:
        ...
        if rows is None:
            ...
            # Same split as `claim check`, and it matters MORE here: this is the surface run at
            # session start, so this is where 22 undifferentiated STALE lines were scrolling past
            # and training the reader to ignore the word.
            sym = [r for r in st if staleness_strength(r) == "symbol"]
            coarse = [r for r in st if staleness_strength(r) == "file"]
            for r in sym:
                print(f"STALE CLAIM {r['id']} — a SYMBOL it cites moved since it was verified "
                      f"({len(r['changed'])} commit(s)); run `info.py claim check` for detail")
            print(f"STALENESS: checked {len(ck)} of {len(rows)} claim(s); {len(sym)} STALE at "
                  f"symbol scope (actionable), {len(coarse)} coarse (a whole FILE they name moved "
                  f"— weak, NOT evidence of rot; narrow with --depends path#symbol), "
